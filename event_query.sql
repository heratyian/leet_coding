...
SELECT events1.event_type, (events1.value - events2.value) AS value
FROM events events1

JOIN events events2
ON events1.event_type = events2.event_type

AND events1.time = (
    SELECT time
    FROM events time1
    WHERE time1.event_type = events1.event_type
    ORDER BY time DESC LIMIT 1
)
AND events2.time = (
    SELECT time
    FROM events time2
    WHERE time2.event_type = events2.event_type
